...
BASE_DIR=$PVC_PATH/$USER/$(dirname $LOCAL_PATH | awk -F'/' '{print $NF}')
kubectl exec $POD_NAME -- bash -c "mkdir -p $BASE_DIR"
kubectl cp "$LOCAL_PATH" "$POD_NAME:$BASE_DIR/$(basename $LOCAL_PATH)"
