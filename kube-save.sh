#!/bin/bash
# Save output data from a Persistent Volume on a Kubernetes cluster. NF-API VERSION

# parse command-line arguments
if [[ $# != 3 ]]; then
	echo "usage: $0 <pvc-name> <remote-path> <workflow_id>"
	exit -1
fi
...
kubectl exec $POD_NAME -- bash -c "for f in \$(find /workspace/_workflows/$WORKFLOW_ID/$(basename $REMOTE_PATH) -type l); do cp --remove-destination \$(readlink \$f) \$f; done"
